...
- headless（autoloop）実行では curl / WebFetch の許可が下りず外部リリース情報を確認できない。検証が必要な変更は (d) としてユーザーに委ねること
- drift-check の PKG コマンド検出は headless PATH（`~/.local/bin` あり・`~/go/bin` なし）で走る。バイナリ配置は `~/.local/bin` を正とすること
